...
textController.saveTitle = (req, res, next) => {
  ...
  models.Text.findOne({ title }).then((data) => {
    if (!data) {
      models.Text.create({ fulltext: fulltext, title: title }).then((text) => {
        res.locals.text = text;
        return next();
      });
    }
    models.Text.findOneAndUpdate({ title }, { fulltext }).then((text) => {});
  });
};

textController.getAll = (req, res, next) => {
  console.log('getAll');
  models.Text.find({}).then((texts) => {
    const titles = [];
